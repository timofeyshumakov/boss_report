    ...
    <div v-show="isLoading" class="loading"> Загрузка...</div>
              ...
              >
                <template v-slot:prepend-item>
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-title>
                        <v-checkbox label="Выбрать все компании" v-model="filters.selectAll.companies" @change="() => toggleSelectAll('companies')" />
                      </v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
                </template>
              ...
              >
                <template v-slot:prepend-item>
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-title>
                        <v-checkbox label="Выбрать все статусы" v-model="filters.selectAll.statuses" @change="() => toggleSelectAll('statuses')" />
                      </v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
                </template>
              ...
              >
                <template v-slot:prepend-item>
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-title>
                        <v-checkbox label="Выбрать все годы" v-model="filters.selectAll.years" @change="() => toggleSelectAll('years')" />
                      </v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
                </template>
              ...
              >
                <template v-slot:prepend-item>
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-title>
                        <v-checkbox label="Выбрать всех пользователей" v-model="filters.selectAll.users" @change="() => toggleSelectAll('users')" />
                      </v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
                </template>
      ...
      <div class="charts-row">
        <div class="chart-container">
          <h3>Сумма сделок по ответственным</h3>
          <Bar
      ...
      <div class="charts-row">
        <div class="chart-container">
          <h3>Распределение сумм по ответственным</h3>
          <Pie
        ...
        <div class="chart-container">
          <h3>Сумма сделок по месяцам (лепестковая)</h3>
          <Radar
...
const toggleSelectAll = (type) => {
  try {
    if (filters.value.selectAll[type]) {
      filters.value.selected[type] = typeof filters.value.value[type][0] === 'object' ? filters.value.value[type].map(item => item.ID) : filters.value.value[type];
    } else {
...
const getTable = (data, uniqueYears) => {
 ...
 const getAllDeals = async (filterEvents, filterStatuses, filterCompaines, filterUsers, dateFrom, dateTo) => {
      let allDeals = [];
      let start = 0;
      ...
      if (filterUsers && filterUsers.length > 0) {
        filters['ASSIGNED_BY_ID'] = filterUsers;
      }

      // НОВОЕ: Добавляем фильтр по дате "с"
      if (dateFrom) {
        filters['>=UF_CRM_1744096783472'] = dateFrom;
      }

      // НОВОЕ: Добавляем фильтр по дате "по"
      if (dateTo) {
        filters['<=UF_CRM_1744096783472'] = dateTo;
      }
...
const getData = async() => {
  try {
    ...
    let localItems = await getAllDeals(filterEvents, filterStatuses, filterCompaines, filterUsers, dateFrom, dateTo);
...
const horizontalBarOptions = {
  ...
  scales: {
    x: {
      beginAtZero: true,
      ticks: {
        callback: (value) => new Intl.NumberFormat('ru-RU', { style: 'currency', currency: 'RUB' }).format(value)
      }
    }
  }
}
...
// НОВОЕ: Watcher для пользователей
watch(
  () => filters.value.selected.users,
  (newVal) => {
    try {
      filters.value.selectAll.users = newVal.length === (filters.value.value.users || []).length;
    } catch (error) {
